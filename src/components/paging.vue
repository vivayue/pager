<template>
  <div class="page-wrap">
    ...
    <div class="page-size" @click="showPageBox($event)" v-click-outside="clickOutSide">
      <span>{{pgsize}}条/页</span>
    </div>
    <div class="pgsize-box" :style="{ left: pgLeft+'px',top: pgTop+'px' }" v-show="showPgSize">
    ...
    <div class="pagination-box">
      <button class="prev" :class="{'disable':current<=1}" type="button" :disabled="current<=1" @click="changePage(current-1)"><i class="left-arrow"></i></button>
      <ul>
        <li v-for="(item,index) in items" 
        :class="{'is-current':current===item.value,'poit':item.text==='···'}" 
        :key="index"
        @click="changePage(item.value)">{{item.text}}</li>
      </ul>
      <button class="next" type="button" :class="{'disable':current>=totalPage}" :disabled="current>=totalPage" @click="changePage(current+1)"><i class="right-arrow"></i></button>
    </div>
  </div>
</template>
